...
| Geo-based DNS | DNS с геомаршрутизацией: резолвер определяет регион клиента и возвращает A record ближайшего POP [[27]](#список-источников). |
...
flowchart TD
    ...
    CDN["CDN<br/>(static, VOD-записи)"]

    subgraph POP["Ближайший POP"]
        VIP["Вход в POP<br/>L4 → L7 nginx"]
        API["api.telemost.ru<br/>(Control API)"]
        WS["ws.telemost.ru<br/>(Signaling)"]
        O["Оркестратор<br/>(Application Based LB, least load)"]
        SFU["SFU-ноды (медиаслой)"]
    end

    U -->|"1. DNS-запрос"| R
    R --> G
    G -.->|"2. A record: IP ближайшего POP"| U
    U -->|"3. HTTPS / WSS на вход POP"| VIP
    VIP --> API
    VIP --> WS
    WS -->|"4. запрос на медиа"| O
    O -->|"5. SFU по загрузке"| WS
    WS -->|"6. IP:port SFU клиенту"| U
    U -.->|"7. медиа SRTP/UDP напрямую"| SFU
    U -->|"static / VOD"| CDN
```

Geo-based DNS не маршрутизирует трафик, а возвращает клиенту A record (IP): вход ближайшего здорового POP для `api`/`ws` и адрес CDN для `static`/VOD. Дальше клиент сам идёт на этот вход (L4 → L7 nginx). TTL ставим 60 секунд, health-check каждые 15 секунд (TCP плюс `GET /health`). Медиа через DNS не проходит: сигнальный сервис запрашивает у оркестратора SFU-нод и возвращает его адрес клиенту, после чего SRTP/UDP идёт напрямую между клиентом и SFU.
...
**Связь с расчётами.** Определяет, как нагрузка раскладывается между POP (раздел глобальной балансировки), читает и пишет состояние в Redis, и задаёт нужное число SFU (раздел списка серверов).

## Simulcast и выбор качества видео
...
**Связь с расчётами.** Задаёт реальный медиатрафик (раздел расчёта нагрузки) и нагрузку на процессор SFU (раздел списка серверов). На базу данных не влияет.
...
**Почему выбрали каскад.** Он держит задержку локальной (каждый общается со своим POP), уменьшает межрегиональный трафик (один поток между POP вместо копии на каждого участника) и снимает потолок ёмкости одного узла, что нужно для крупных вебинаров.

**Связь с расчётами.** Снижает межрегиональный трафик (раздел глобальной балансировки) и определяет, сколько SFU нужно и как POP связаны между собой (раздел списка серверов). На базу данных не влияет.
