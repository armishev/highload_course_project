...
flowchart TD
    ...
    U -->|"1. DNS-запрос"| R
    R --> G
    G -.->|"2. DNS-ответ: VIP ближайшего POP"| U
    U -->|"3. HTTPS / WSS на VIP"| VIP
    VIP --> API
    VIP --> WS
...
Geo-based DNS не маршрутизирует трафик, а возвращает клиенту адрес: VIP ближайшего здорового POP для `api`/`ws` и адрес CDN для `static`/VOD. Дальше клиент сам идёт на VIP (L4 → L7 nginx). TTL ставим 60 секунд, health-check каждые 15 секунд (TCP плюс `GET /health`). Медиа через DNS не проходит: сигнальный сервис запрашивает у оркестратора SFU-нод и возвращает его адрес клиенту, после чего SRTP/UDP идёт напрямую между клиентом и SFU.
