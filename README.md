...
Раздел описывает глобальную балансировку (Global Scale Load Balancing, GSLB), то есть распределение входящего трафика между дата-центрами. Балансировка внутри ДЦ (LSLB) вынесена в следующий раздел.
...
| Media / Control / Signaling | Три плоскости видеосервиса с разным профилем нагрузки: медиа (RTP, сотни Гбит/с, чувствительно к задержке), управление (REST, низкий RPS), сигналинг (WebSocket, вход в звонок и контроль). |
...
> **Почему контуры разделены по доменам:** у них принципиально разные профили нагрузки. Control API даёт редкие мелкие REST-запросы, Signaling держит долгие WebSocket-соединения, TURN гоняет UDP-relay, а статика и VOD уходят на CDN. Разделение позволяет масштабировать их независимо и держать отдельные TLS- и upstream-политики.
...
> **Почему записи идут через CDN, а медиа нет:** live-медиа интерактивно (целевая задержка до 150 мс, см. ниже) и проходит через SFU. Запись встречи (VOD) к задержке нечувствительна, это обычный статический файл, который раздаёт CDN.
...
> **Почему именно эти четыре города:** доли привязаны к численности населения федеральных округов по Росстату, всего около 146.2 млн [[23]](#список-источников). Москва обслуживает центр и юг, поскольку отдельной южной площадки нет и ЮФО с СКФО маршрутизируются на неё. Екатеринбург закрывает Поволжье и Урал, Новосибирск отвечает за Сибирь и Дальний Восток, Санкт-Петербург за Северо-Запад.

> **Почему primary в Москве:** это крупнейшая точка межоператорского обмена трафиком в РФ (MSK-IX) [[25]](#список-источников), что даёт лучшую связность с операторами и минимальную сетевую задержку для контрольной плоскости.
...
Эти значения идут на вход в расчёт парка SFU и TURN по каждому POP в разделах локальной балансировки и списка серверов.
...
flowchart TD
    ...
    subgraph POPs["4 POP: МСК / ЕКБ / НСК / СПб"]
        ...
        SFU["SFU-ноды<br/>(медиаплоскость)"]
    end
...
Вход (`api`, `ws`, `static`) балансируется Geo-based DNS в ближайший здоровый POP. TTL ставим 60 секунд, health-check каждые 15 секунд (TCP плюс `GET /health`). Медиапоток через DNS не проходит: signaling запрашивает у оркестратора конкретный SFU-нод, и SRTP/UDP идёт напрямую между клиентом и SFU.

> **Почему оговорка про резолвер важна:** Geo-based DNS видит IP резолвера, а не клиента, поэтому пользователь публичного резолвера (8.8.8.8) может попасть в неоптимальный POP. Проблему смягчает расширение EDNS Client Subnet [[27]](#список-источников). Даже если вход всё же ушёл не в тот POP, оркестратор аллоцирует медиа по фактической близости и нагрузке.

## Схема Anycast-балансировки

Anycast не используется по двум причинам:

1. нужен собственный AS-номер и сеть /24 с координацией BGP, что избыточно для масштаба MVP [[28]](#список-источников);
2. при перескоке трафика между ДЦ Anycast рвёт соединения, а для видеозвонка это обрыв медиасессии (прилипание долгоживущего UDP-потока к одному узлу не гарантируется), что для real-time неприемлемо.

> **Почему без Anycast не теряем в качестве:** его роль закрывают Geo-based DNS на входе и Application Based allocation для медиа. Второй механизм точнее: он выбирает узел по реальной загрузке (least load), а не только по топологии, и не рвёт сессии.

## Механизм регулировки трафика между ДЦ

1. **Allocation (Application Based LB).** При входе signaling обращается к оркестратору, и тот по алгоритму least load назначает участнику наименее загруженный SFU-нод ближайшего POP. Встреча «прописывается» (homed) на POP инициатора или большинства участников.
...
| STUN / TURN | STUN сообщает клиенту его внешний адрес для NAT, TURN ретранслирует медиа, когда прямое соединение с SFU невозможно (симметричный NAT, firewall). |
| Sticky WebSocket | Все пакеты одного WS-соединения идут на один backend (`hash` по адресу клиента), потому что соединение живёт часами. |
...
flowchart TD
    ...
    L7A --> API
    L7A --> WS
    C -. "SRTP/UDP напрямую, минуя L4/L7" .-> SFU
    C -. "relay при NAT" .-> TURN
    C -->|"static / записи"| CDN
```

> **Почему медиа минует L4/L7:** через балансировщики идёт только сигналинг и REST. Медиапоток (587.1 Гбит/с в пике) течёт напрямую клиент↔SFU по UDP, а SFU выбирает оркестратор (см. предыдущий раздел). Прогонять такой объём через L7 бессмысленно и дорого, плюс TLS termination к UDP-медиа неприменим. Поэтому пропускная способность сети не является лимитером L7, она приземляется на SFU и TURN.
...
Долю сессий, которым нужен relay через TURN, принимаем **10 % (консервативно)**. В нашей SFU-топологии клиент подключается не к другому клиенту, а к серверу с публичным IP [[26]](#список-источников), поэтому прямое соединение через server-reflexive кандидата проходит в большинстве случаев, и доля relay ниже, чем в P2P-mesh. Пиковый relay-трафик: `587.1 · 0.10 ≈ 58.7 Гбит/с`.

Подробный расчёт парка TURN- и SFU-узлов (по пропускной способности сети, с резервированием) вынесен в раздел «Список серверов».
...
- **Sticky WebSocket.** Сигнальное WS-соединение живёт всю встречу, поэтому L7 закрепляет его за одним backend по `hash $remote_addr consistent`. Без этого участник терял бы roster-состояние при перебалансировке.

## Внутренняя балансировка

Межсервисный трафик (signaling → orchestrator → БД, recording-воркеры) балансируется средствами Kubernetes: ClusterIP-сервисы и kube-proxy, service discovery через k8s. Отдельный L7 для внутренних вызовов не разворачиваем, чтобы не плодить хопы.
...
Крупные встречи (вебинары до 1000 участников) это главный источник горячих ключей. На стороне медиа их разносит каскадирование SFU (см. раздел глобальной балансировки), на стороне данных roster-обновления отдаются участникам батчами, а не по одному событию на каждого.

---

# Физическая схема БД

Здесь логические сущности привязаны к конкретным СУБД, шардингу и репликации. Ключевое наблюдение: на реляционную БД приходится лишь сотни запросов в секунду в пике (см. нагрузку в предыдущем разделе), а ~0.6 ТБ метаданных умещаются в один узел. Основной RPS (heartbeat и meeting control) идёт в Redis, поток телеметрии в ClickHouse, а главная тяжесть хранения это ~2.7 ПБ записей в object storage. Поэтому модель масштабирования подбирается под задачу [[32]](#список-источников), а не по шаблону.
...
Здесь описаны три алгоритма, способ реализации которых заметно влияет на нагрузку и устройство системы. Все они относятся к медиаплоскости и оркестратору, то есть к самой нагруженной части видеосервиса.

## Выбор SFU-узла для участника

**Где применяется.** Оркестратор, в момент входа человека во встречу.

**Задача.** Когда участник заходит в звонок, нужно решить, на какой именно медиасервер (SFU) его подключить. Сервер должен быть и близко к человеку (чтобы задержка была маленькой), и не перегружен. Если выбрать плохо, у части людей будет лагать видео.
...
- Hash по `meeting_id`: участники прилипают к одному узлу, но крупная встреча может его перегрузить.

**Почему выбрали least load.** Он смотрит на фактическую загрузку узлов, держит задержку низкой за счёт ближайшего POP и нормально уживается с каскадированием.
...
5. Если канал получателя ухудшился, SFU молча переключает его на версию ниже.
...
- Один поток без simulcast: просто, но либо у слабых тормозит, либо всем приходится резать качество.
- SVC (масштабируемое кодирование): эффективнее simulcast, но сложнее и хуже поддерживается клиентами.

**Почему выбрали simulcast.** SFU только выбирает готовую версию и пересылает её, ничего не перекодируя, поэтому процессор почти не нагружается. Каждый получает посильное качество. Именно это объясняет, почему в расчёте нагрузки downlink участника фиксированный: клиент получает ограниченный набор плиток нужного качества, а не все потоки в максимальном разрешении.
...
**Где применяется.** Медиаплоскость, встречи с участниками из разных регионов и большие вебинары (до 1000 человек).

**Задача.** Если в одной встрече есть люди из Москвы, Новосибирска и Питера, то посадить всех на один SFU нельзя: половина получит большую задержку. Но и слать каждый поток между регионами отдельной копией для каждого получателя тоже плохо, потому что трафик между POP умножается.
...
Сводная таблица стека по областям. Медиаплоскость (SFU, TURN) вынесена отдельно, потому что это самая нагруженная и специфичная для видеосервиса часть.
...
| Kotlin | Android-клиент | нативный доступ к камере, микрофону и фоновому режиму, лучше тянет звонок, чем веб в обёртке |
| Swift | iOS-клиент | нативная интеграция с системой (CallKit, PushKit, энергосбережение) |

## Медиаплоскость и WebRTC
...
Надёжность строится на двух принципах: резервирование каждого компонента (схема N+1, а для хранилищ репликация) и распределение по зонам доступности внутри POP [[40]](#список-источников), чтобы отказ одной зоны не ронял сервис целиком. Ключевая особенность видеосервиса: идущий звонок живёт на медиаплоскости и не зависит от реляционной БД, поэтому многие отказы он переживает без обрыва.
...
- **failover primary PostgreSQL** (30–60 с) не роняет идущие звонки, потому что медиаплоскость не обращается к реляционной БД во время звонка.
...
flowchart TD
    ...
    subgraph MEDIA["Медиаплоскость (по POP, каскад между POP)"]
    ...
    API --> RD
    SIG --> RD
    SFU -.->|"запись/аудио"| KAFKA
    KAFKA -.-> REC
    KAFKA -.-> ASR
    REC -.-> OS
    ASR -.-> OS
    REC -.->|"метаданные"| PG
    C -.->|"QoS-телеметрия"| KAFKA
    KAFKA -.-> CH
    C -->|"static / VOD"| CDN
    OS -.-> CDN
```
...
**Вход во встречу.** Клиент resolved через DNS, поднимает WebSocket к signaling, тот спрашивает оркестратор о ближайшем свободном SFU (состояние читается и пишется в Redis), и отдаёт клиенту адрес узла. Метаданные встречи и участника пишутся в PostgreSQL.
...
**Запись.** SFU-узел публикует задание в Kafka, recording-воркер сводит и кодирует видео (ffmpeg), кладёт файл в Ceph (S3), а метаданные в PostgreSQL. Готовая запись раздаётся через CDN.

**Расшифровка.** Аудио уходит в Kafka, ASR (Yandex SpeechKit) переводит речь в текст асинхронно после встречи, результат сохраняется в Ceph.

**Телеметрия.** Клиенты шлют метрики качества (packet loss, jitter, RTT) в Kafka, оттуда поток попадает в ClickHouse для аналитики. Этот путь фоновый и не влияет на сам звонок.

---

# Список серверов

Парк считается от удельных характеристик одного узла. Профиль узлов берётся из лекции по аппаратному обеспечению [[40]](#список-источников): 2U-сервер, 2 CPU, NVMe, сетевые карты 2×25 или 2×10 Гбит/с.
...
Парк закономерно для видеосервиса доминируют медиа, кодирование записей и хранилище (SFU 65, Recording 33, Ceph OSD 22), а реляционная БД скромная (3 узла). Итог 178 серверов превышает порог методички в 100 серверов.
