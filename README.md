...
> **Почему контуры разделены по доменам:** у них принципиально разные профили нагрузки. Control API даёт редкие мелкие REST-запросы, Signaling держит долгие WebSocket-соединения, TURN обрабатывает UDP-relay, а статика и VOD уходят на CDN. Разделение позволяет масштабировать их независимо и держать отдельные TLS- и upstream-политики.
...
2. при перескоке трафика между ДЦ Anycast рвёт соединения, а для видеозвонка это обрыв медиасессии (закрепление долгоживущего UDP-потока за одним узлом не гарантируется), что для real-time неприемлемо.
...
В составе серверов закономерно для видеосервиса доминируют медиа, кодирование записей и хранилище (SFU 65, Recording 33, Ceph OSD 22), а реляционная БД скромная (3 узла). Итог 178 серверов превышает порог методички в 100 серверов.
